...
Item {
  ...
  // Prefer the shared AppLibrary, but cloned plugins can receive an empty
  // facade. Fall back to DesktopEntries for the QML-native apps provider.
  function mergeAppRows() {
    var rows = root.appLibrary ? root.appLibrary.sortedEntries("") : null
    var fromLibrary = rows && rows.length > 0
    if (!fromLibrary) rows = DesktopEntries.applications.values || []
    var appRows = []
    for (var j = 0; j < rows.length; j++) {
      var entry = fromLibrary ? rows[j].entry : rows[j]
      if (!entry || entry.noDisplay) continue
      var label = fromLibrary ? root.appLibrary.entryName(entry) : String(entry.name || entry.id || "")
      if (!label) continue
      var appId = String(entry.id || "")
      if (!appId) continue
      var subtext = fromLibrary ? root.appLibrary.entrySubtext(entry) : String(entry.genericName || "")
  ...
  function openDmenu(payload) {
    ...
    dmenuWidth = Math.max(1, Number(payload.width || 420))
